...
class BasisFunctor f b b' where
  fmapB' :: (Eq r, Num r) => (b -> b') -> f b -> FreeModule r (f b')
...
fmapB :: (Eq r, Num r, Ord (f b'), BasisFunctor f b b') =>
...
class R.MonadR f b' => BasisMonad f b b' where
  bindB' :: (Eq r, Num r) => (b -> FreeModule r (f b')) -> f b -> FreeModule r (f b')

injectB :: (Num r, R.MonadR f b) => b -> FreeModule r (f b)
injectB = inject . R.return

bindB :: (Eq r, Num r, Ord (f b'), BasisMonad f b b') => FreeModule r (f b)
...
include :: (Eq r, Num r, Ord (f b), R.MonadR f b)
