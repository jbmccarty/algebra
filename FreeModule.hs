...
instance (Eq r, Show r, Num r, Show b) => Show (FreeModule r b) where
  showsPrec d = showsPrecFM d . unpack

-- FreeModule r b is an abelian group
instance (Eq r, Num r, Ord b) => AbelianGroup (FreeModule r b) where
  (.+.) = lift2 addFM
  zero = pack zeroFM
  negateA = lift negateFM

-- FreeModule r b is an r-module
instance (Eq r, Num r, Ord b) => Module r (FreeModule r b) where
  r .* x = lift (actFM r) x
...
instance (Eq r, Num r, Ord b') => R.Functor (FreeModule r) b b' where
...
instance (Eq r, Num r, Ord b') => R.Monad (FreeModule r) b b' where
...
returnList :: (Eq r, Num r, Ord b) => [(b, r)] -> FreeModule r b
