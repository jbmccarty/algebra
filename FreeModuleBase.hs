...
showsPrecFM :: (Eq r, Num r, Show r, Show b) => Int -> FM r b -> ShowS
...
canonicalizeFM :: (Eq r, Num r, Ord b) => FM r b -> FM r b
canonicalizeFM = M.filter (/= 0)

addFM :: (Eq r, Num r, Ord b) => FM r b -> FM r b -> FM r b
...
actFM :: (Eq r, Ord b, Num r) => r -> FM r b -> FM r b
actFM r = canonicalizeFM . M.map (r*)

-- FM is a (restricted) functor on the basis type
fmapFM :: (Eq r, Num r, Ord b') => (b -> b') -> FM r b -> FM r b'
...
bindFM :: (Eq r, Num r, Ord b') => FM r b -> (b -> FM r b') -> FM r b'
